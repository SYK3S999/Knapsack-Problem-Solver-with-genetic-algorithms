...
def clear_form():
    """Callback to clear the form inputs."""
    st.session_state['new_item_name'] = ""
    st.session_state['new_item_weight'] = 1
    st.session_state['new_item_value'] = 1
...
def item_management_section():
    ...
    with col4:
        ...
        if st.button("Add Item", type="primary"):
            if add_item(name, weight, value):
                clear_form()  # Clear inputs after adding
    ...
    if st.session_state['items']:
        ...
        # Display items with delete button beside each item
    ...
    if st.button("Solve Knapsack Problem", type="primary", disabled=len(st.session_state['items']) == 0):
        ...
        with st.spinner("🧮 Running genetic algorithm..."):
            try:
                ...
                st.error("""⚠️ Couldn't connect to the backend server. Make sure it's running on port 5000.""")
