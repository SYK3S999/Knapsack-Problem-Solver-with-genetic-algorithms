...
def item_management_section():
    ...
    if 'form_value' not in st.session_state:
        st.session_state.form_value = 1
    ...
    with st.form(key="item_form"):
        ...
        with cols[0]:
            name = st.text_input("Item Name", key='form_name')
        with cols[1]:
            weight = st.number_input("Weight", min_value=1, key='form_weight')
        with cols[2]:
            value = st.number_input("Value", min_value=1, key='form_value')
        with cols[3]:
            st.write("")  # Spacing
